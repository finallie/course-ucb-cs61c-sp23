...
int main(int argc, char **argv)
{
    ...
    if (procID == 0)
    {
        ...
        while (nextTask != numTasks)
        {
            ...
            MPI_Send(&nextTask, 1, MPI_INT32_T, src_id, 0, MPI_COMM_WORLD);
        ...
        for (int i = 0; i < totalProcs - 1; i++)
        {
            ...
            message = TERMINATE;
            MPI_Send(&message, 1, MPI_INT32_T, src_id, 0, MPI_COMM_WORLD);
        }
    }
    else
    {
        ...
        while (true)
        {
            ...
            MPI_Recv(&message, 1, MPI_INT32_T, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
